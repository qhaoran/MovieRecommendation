...
testing_small = sc.textFile("./%s" % testFile).map(lambda line: line.split(",")).filter(
    lambda line: not line[0].isalpha()).map(
    lambda line: ((int(line[0]), int(line[1])), 1))
rating = sc.textFile("./%s" % ratingFile).map(lambda line: line.split(",")).filter(
    lambda line: not line[0].isalpha()).map(
...
# write result in txt
predicted_ratings = predicted_ratings.collect()
f = open("Haoran_Que_result_task2.txt", "w")
